...
#' @description
#' Arranges patients by sequencing technique (R Package required: 'dplyr',
#' 'reshape2, 'seriation')
#'
...
#' @param method_dist Character string with the name of the method to calculate
#' the distance matrix
...
order_patient <- function(
  ...
  # add column ae_count which includes the number of days between day_start
  # and day_end
  tmp <- ae_data %>%
    dplyr::mutate(ae_count = .data$day_end - .data$day_start + 1)
  ...
  tmp <- tmp %>%
    dplyr::mutate(
      ae_count_sev1 = ifelse(.data$sev == 1, .data$ae_count, 0),
      ae_count_sev2 = ifelse(.data$sev == 2, .data$ae_count, 0),
      ae_count_sev3 = ifelse(.data$sev == 3, .data$ae_count, 0)
    )
  ...
  tmp3 <- tmp2 %>%
    ...
    dplyr::mutate(ae = paste(.data$ae, .data$tmp, sep = "_")) %>%
    dplyr::select(-"tmp") %>%
    tidyr::spread("ae", "val", fill = 0, sep = "_")

  #rename column names
  for (i in seq_along(colnames(tmp3)[-1])) {
    if (length(strsplit(colnames(tmp3)[i + 1], "_")[[1]]) == 5) {
      colnames(tmp3)[i + 1] <-
        paste(strsplit(colnames(tmp3)[i + 1], "_")[[1]][c(2, 5)],
              collapse = "_")
    } else {
      colnames(tmp3)[i + 1] <-
        paste(strsplit(colnames(tmp3)[i + 1], "_")[[1]][c(2)],
              collapse = "_")
    }
  }
  ...
  tmp4 <- tmp3 %>%
    dplyr::right_join(
      patients %>%
        dplyr::mutate(death = ifelse(.data$death == 99999, 0, 1)) %>%
        dplyr::select("ps", "end", "death", "ae_frequency"),
      by = c("patient" = "ps")
    )
  ...
  tmp5$"SEQUENCING" <- seq_len(nrow(tmp5))
